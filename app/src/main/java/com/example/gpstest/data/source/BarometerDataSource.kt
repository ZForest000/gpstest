...
class BarometerDataSourceImpl(
    ...
    override fun hasBarometer(): Boolean = sensorManager?.getDefaultSensor(Sensor.TYPE_PRESSURE) != null

    override fun getPressure(): Flow<Float?> =
        callbackFlow {
    ...
    override fun getAltitude(seaLevelPressure: Float): Flow<Double?> =
        callbackFlow {
            val sensorManager = sensorManager
            val pressureSensor = sensorManager?.getDefaultSensor(Sensor.TYPE_PRESSURE)
